...
5. **错字校对**：调用 `sumeru-polish` 对**所有已审查章节**进行错字校对（错别字、误用字词、人名地名前后不一致、明显笔误，严禁润色改写）
6. **完稿校验**：调用 `sumeru-finalize` 对**所有已校对章节**完成错别字检查、标点规范、逻辑漏洞最终排查与作品打包
...
[review 已自动完成大纲修订+章节重写+轻量修复，直接修改了 chapters/（自动备份到 .sumeru/write/original/）]
    ↓
[sumeru-polish] 错字校对（**校对所有章节，每个Agent最多3章**）
    → 输入: chapters/*.md, .sumeru/review/*.json
    → 输出: chapters/*.md（校对后直接修改，自动备份到 .sumeru/write/original/）, .sumeru/polish/*.json
    → ✅ **必须完成全本校对**
    → 📝 **校对结果直接修改 chapters/，修改前自动备份到 .sumeru/write/original/**
    → ⚠️ **遵循全局3章/Agent约束**
    ↓
[阶段检查点 4] 验证校对完成
    ↓
...
6. **审查阶段**：调用 `sumeru-review` 执行三阶段审查修复流程（遵循全局3章/Agent约束）：
    ...
    - 修复完成后重新验证，确保所有问题已解决
    - 完成后更新进度文件
7. **校对阶段**：调用 `sumeru-polish` 进行全本错字校对（遵循全局3章/Agent约束），只修正错别字、误用字词、人名地名前后不一致、明显笔误，严禁润色改写
    - 校对完成后，直接修改 `chapters/` 中的文件（修改前自动备份到 `.sumeru/write/original/`）
    - 校对过程和结果记录到 `.sumeru/polish/` 目录
    - 完成后更新进度文件
8. **完稿阶段**：调用 `sumeru-finalize` 对所有校对后的章节进行完稿校验（遵循全局3章/Agent约束），从 `chapters/` 目录读取最终内容（已包含review修复和polish校对后的结果），输出多种格式（Markdown/HTML/EPUB）到 `publish/` 目录，生成创作总结报告，完成后更新进度文件为最终完成状态
